...
object implicitHints:
  ...
  private  def memberFields(using Quotes)(symbol: quotes.reflect.Symbol) =
    try symbol.memberFields catch { case NonFatal(_) => List.empty }

  private  def memberMethods(using Quotes)(symbol: quotes.reflect.Symbol) =
    try symbol.memberMethods catch { case NonFatal(_) => List.empty }
  ...
  private def findImplicits(using Quotes)(
    ...
    while queue.nonEmpty do
      ...
      if visited.add(symbol) && mightBeScalaSource(symbol) then
        memberFields(symbol) ++ memberMethods(symbol) foreach { member =>
          try
  ...
  private def associatedImplicitScope(using Quotes)(
    ...
    val baseCompanions =
      tpe.dealias.baseClasses.distinct map { _.companionModule }

    val implicitScope =
      (baseCompanions ++ prefixes).toSet - Symbol.noSymbol
  ...
  private def associatedImplicitScopeExtension(using Quotes)(
    ...
    scope flatMap { symbol => memberFields(symbol) filter { _.isImplicit } }
  ...
  def mightBeResolvable(using Quotes)(tpe: quotes.reflect.TypeRepr): Boolean =
    import quotes.reflect.*
  ...
  private def importStatement(using Quotes)(ref: quotes.reflect.Select) =
    ...
    def makePath(term: Term): List[String] =
      term match
        case Select(qualifier, name) =>
          if term.symbol == base.symbol then makePath(qualifier) else makePath(qualifier) :+ name
  ...
  def conversions(using Quotes)(tpe: quotes.reflect.TypeRepr): String =
    import quotes.reflect.*
          ...
          map { (entries, relatedHint, importStatement) =>
            val identifiers = (entries flatMap { (ref, _, _) =>
              if ref.symbol.isExtensionMethod then
                ...
                memberMethods(ref.tpe.widenTermRefByName.finalResultType.typeSymbol) collect {
                  case symbol if isMeaningfulMethod(symbol) && !(symbol.name contains '$') =>
                    symbol.name
                }
            }).distinct.sorted
